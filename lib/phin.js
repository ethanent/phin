...
const phin = async (opts) => {
	...
	if (typeof opts.core === 'object') {
		Object.entries(opts.core).forEach(([key, val]) => req.option(key, val))
	}

	const res = await req.send()

	if (Object.prototype.hasOwnProperty.call(res.headers, 'location') && opts.followRedirects) {
...
phin.defaults = (defaultOpts) => async (opts) => {
	...
	return await phin(Object.assign({}, defaultOpts, nops))
}
