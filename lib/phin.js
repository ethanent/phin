...
* @property {boolean} [body=true] - Concatenates the response sent by the server into the <code>body</code> property of the response object
* @property {boolean} [stream=false] - Injects the 'stream' property into the response object and sets the 'body' property to <code>false</code>
...
* @param {?http.serverResponse} phinResponse - phin response object. Like <a href='https://nodejs.org/api/http.html#http_class_http_serverresponse'>http.ServerResponse</a> but has a 'body' property containing the response body (unless option is disabled) and a 'stream' property containing the response stream (if option is enabled)
*/
...
const phin = (opts, cb) => {
	...
	if (options.stream) {
		...
		options.body = options.body === false ? false : true
	}
